...
class UpdateStudentController {
  async handle(req: Request, res: Response) {
    const { name, password, teachingTypeId, isOnline, profilePicture } = req.body as IStudentRequest;

    const { email } = req.params;

    const updateStudentService = new UpdateStudentService(new StudentRepository());

    await updateStudentService.execute({ email, name, password, teachingTypeId, isOnline, profilePicture });
